...
6. Desde `src/backend`, ejecuta `python -m app.cli gmail-auth`. Se abrirá el navegador para el consentimiento manual y se guardará el refresh token en `SUPERMARKET_GMAIL_TOKEN_PATH` (por defecto `./secrets/gmail-token.json`).
...
La consulta de backfill es `python -m app.cli sync --all`; las siguientes
ejecuciones usan `python -m app.cli sync --since-last`. Los mensajes no se
